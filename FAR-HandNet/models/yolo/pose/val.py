...
from ultralytics.utils.metrics import OKS_SIGMA, PoseMetrics, box_iou, kpt_iou, OKS_SIGMA_HAND
...
class PoseValidator(DetectionValidator):
    ...
    def init_metrics(self, model):
        ...
        # self.sigma = OKS_SIGMA if is_pose else OKS_SIGMA_HAND
        # self.sigma = OKS_SIGMA if is_pose else np.ones(nkpt) / nkpt
        self.sigma = OKS_SIGMA_HAND


    def GetSigma(self,gt_kpts,pre_kpts,area):
        ...
        return torch.tensor(sigma_all).unsqueeze(1) if nbox > 1 else torch.tensor(sigma_all)
    ...
    def _process_batch(self, detections, labels, pred_kpts=None, gt_kpts=None):
        ...
        if pred_kpts is not None and gt_kpts is not None:
            ...
            # sigma =self.GetSigma(gt_kpts, pred_kpts, area)
